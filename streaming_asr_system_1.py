...
class StreamBatch:
    ...
    def add_chunk(self, chunk: AudioChunk, frames) -> bool:
        """Add chunk to batch. Returns True if batch is ready."""
        #self.pending_chunks.append(chunk)
        
        self.pending_chunks.append((chunk, frames))
...
class StreamingASREngine:
    ...
    def process_chunk(self, stream_id: str, audio_chunk: np.ndarray) -> str:
        ...
        logger.debug(f"Enqueuing chunk {chunk.chunk_id} for stream {stream_id} at {chunk.timestamp}")
        try:
            self.processing_queue.put(chunk, timeout=0.1)
            logger.debug(f"Chunk {chunk.chunk_id} enqueued successfully")
        except queue.Full:
    ...
    def _processing_loop(self):
        ...
        while self.is_running:
            try:
                ...
                logger.debug(f"Dequeued chunk {chunk.chunk_id} from stream {chunk.stream_id}")
            except queue.Empty:
                continue
            # Add chunk to batch, check if batch ready
            ready = self.batch_manager.add_chunk(chunk)
            logger.debug(f"Added chunk {chunk.chunk_id} to batch, batch size {len(self.batch_manager.pending_chunks)}, ready={ready}")
            if not ready:
                continue
            ...
            with torch.no_grad():
                ...
                logger.debug(f"Received {len(transcripts)} transcripts from ASR model")
